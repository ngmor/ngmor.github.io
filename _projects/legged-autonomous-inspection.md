...
I was interested in this autonomous inspection application, so my winter project in Northwestern University's MSR program focused on exploring this with the quadruped [**Unitree Go1**](https://www.trossenrobotics.com/unitree-overview).
...
### Navigation
A critical task for this project is autonomous navigation around a mapped environment and avoiding obstacles that may not have been present in the original map. Luckily ROS 2 has [**Nav2**](https://navigation.ros.org/), a software stack designed specifically for this task. This integrated with a **3D LiDAR** and custom C++ control packages to allow the Go1 to navigate through its environment autonomously.
...
The second required subsystem is visual text detection and recognition, referred to here as **Optical Character Recognition (OCR)**. This project uses the Go1's onboard stereo cameras and two pre-trained machine learning models to accomplish this task. I wrote a [ROS 2 C++ wrapper](https://github.com/ngmor/unitree_camera) for the [**Unitree Camera SDK**](https://github.com/ngmor/UnitreecameraSDK) to publish camera data. This data then feeds into an [**EAST Text Detection**](https://arxiv.org/abs/1704.03155v2) model to locate text in the images. Finally, a **convolutional recurrent neural network (CRNN)** is used to recognize and interpret the text data.
